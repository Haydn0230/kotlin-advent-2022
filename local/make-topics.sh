...
# standardTopics=(
# 	gstp.sportsbook.catalogue.local.updates.avro
# 	)
...
for topic in "${compactedTopics[@]}"; do
  ...
  docker-compose exec broker kafka-topics --bootstrap-server $BROKER_HOST --create --topic "$topic" --partitions 4 --replication-factor 1 --config cleanup.policy=compact --config compression.type=gzip
  echo "👍"
done
